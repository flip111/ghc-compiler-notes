...
defaultAppContext = appContext $
  AppContextCommon
    ...
    , sFlagsGeneralOptions =
        [ (True, Flags.Opt_Haddock)
        , (True, Flags.Opt_KeepRawTokenStream)
        ]
    , sFlagsGlobalIncludePaths =
        [ "dummy_includes"
        , "output/ghc/compiler"
...
instance Monad m => HasSource "AppContext" AppContext (AppT m) where
  await_ _ = AppT (\ctx -> pure ctx)
  {-# INLINE await_ #-}
...
instance Monad m => HasSource "envSession" Session.DynFlags (AppT m) where
  await_ _ = AppT (\ctx -> pure (envSession ctx))
  {-# INLINE await_ #-}
